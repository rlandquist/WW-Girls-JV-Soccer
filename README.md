...
```js
// sw.js, near the top
const CACHE_VERSION = 'v13';   // bump to 'v14', 'v15', etc.
```

Current version: `v13` (May 2026 — fixed patterns missing from downloaded PNGs on the Card, Schedule, and Roster tools. Only `streaks` (a plain `linear-gradient`) was actually surviving the html2canvas export; the other 8 patterns used `repeating-linear-gradient` (carbon / grid / slash / diamonds), `radial-gradient` (halftone / noise), or `repeating-radial-gradient` (pulse), all of which html2canvas 1.4.1 silently drops. `makeSoccerPatterns` in `common.js` now pre-renders those 7 patterns to small canvas tiles and returns a CSS `url()` pointing at the tile's PNG data URL — html2canvas paints data-URL `background-image` natively, so the patterns survive the export end-to-end. Streaks stays as a plain `linear-gradient`; clean stays as `'none'`. No HTML files touched — the fix is entirely inside `common.js`'s `makeSoccerPatterns`. Skipped `v12` because a failed `onclone`-style fix briefly shipped under that label and may still be cached on devices that hit the site in that window; bumping straight to `v13` evicts both stale `v11` and `v12` caches in one step. `v11` was the May 2026 cleanup that removed decorative diagonal stripes from the Preview and Halftime cards: both the upper-right `.card-stripe` pair on each card body and the upper-left `.gametime-stripe` / `.nexthalf-stripe` inside each card's footer block. Score (Final) card untouched — its `.photo-stripe` pair does structural work blending the photo edge into the navy score column; the diagonal-line SVG `background-image` on `.card-next` / `.card-gametime` / `.card-nexthalf` footers is preserved. Brings Preview and Halftime's visual model in line with Schedule and Roster: one base colour + one full-card pattern via `applyPatternToElement`, no extra decorative overlays. `v10` was Card's PNG-export error toasts on download() and copyToClipboard() switching to friendlier per-tool wording via the new `errorMessage` opt on `WWCommon.downloadElementAsPng` and `copyElementAsPngToClipboard`, plus a cosmetic comment-block slim on Schedule and Roster's bottom-of-style chrome; `v9` pruned Card.html's mobile `@media` block in favour of `shared.css` (~3.6 KB lighter); `v8` rewrote Card's `download()` and `copyToClipboard()` as thin wrappers around `WWCommon` with new `filename`/`before`/`after` hooks plus an orphan-comment sweep across Card/Schedule/Roster/Goals; `v7` was Card tool's pattern rendering moved from canvas drawing to CSS-background overlays via `WWCommon.applyPatternToElement`, matching how Schedule and Roster already render their patterns; the extended `makeSoccerPatterns` helper gained `accentRgb` and `light:true` options so the same helper drives all three card modes including Preview's light-bg flip; `v6` added `shared.css` plus a suite of shared helpers in `common.js` (`makeSoccerPatterns`, `applyPatternToElement`, `downloadElementAsPng`, etc.); `v5` was the Card scorer-display split per-card-mode; `v4` added Classic 8 + photo-credit logos to the pre-cached shell; `v3` was the unified app-shell across Card / Schedule / Roster; `v2` added the Goals tool to the shell). Bumping forces a clean install on first reload; without a bump, returning users still pick up the new shell on their second reload via stale-while-revalidate.
